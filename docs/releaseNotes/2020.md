...
-   **User Experience Improvements**
    ...
    -   **Updated** Product for Zscaler branding. Updated login page logo, login
        text from Cloudneeti to Zscaler CSPM (Cloud Security Posture Management).
        Updated logo on the portal top left from Cloudneeti to Zscaler.

    -   **Updated** the 'Billing Dashboard page' to download billing details and
        billed resources at an Cloud Account and License level - CSV report

    -   **Updated** Summary Word report to download for Private Benchmarks Summary
        Report.

    -   **Updated** the ‘Onboarding Health Status’ page to now include the status of
        Azure Service Principal, M365 Service Principal, and Partner Admin Link (PAL). [Refer documentation](../../administratorGuide/manageAccounts/#onboarding-health-status-preview){target=_blank}

    -   **Added** support for Rapid7 Azure Vulnerability Solutions. Enabling
        deployment of vulnerability assessment solutions using Azure Security Center
        allows us to collect all the Vulnerability assessments reported by partner
        solutions and associate them with each of the cloud assets. [Refer documentation](../../onboardingGuide/azureVulnerability/#72-install-vulnerability-solution-on-vms){target=_blank}

-   **Platform & Stability Improvements**

    -   Scale, and stability improvements in data collection and processing by using
        Autoscale feature COSMOS DB

    -   Fixed following bugs

        -   Cloud Trail policies are not getting remediated in multi-account remediation
        ...
        - Unable to download word report for Marketplace license
        - Word report is not getting downloaded for Marketplace Paid licenses
