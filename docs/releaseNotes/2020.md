...
1.  **User Experience Improvements**
    ...
    -   **AWS Remediation:** Support for Cloudneeti Remediation Framework different
        region deployment.
...
2.   **Platform & Stability Improvements**
    ...
    -   Fixed following bugs
        ...
        -   Other resources in a policy are not getting remediated if any one
            resource fails to remediate due to some reason

        -   UI/UX: Msal.idtoken is accepted by cloudneeti api endpoints even after
            it is expired

        -   Resource Not Found in CreatePrivateBenchmarkSummaryActivity (QA, Prod)

        -   Exception in RBACCustomerPolicySummaryActivity

        -   Missing TTL in AggregatedCISOTrendData AggregatedCISODashboard
...
3.   **Policies & Benchmarks**
    ...
    -   Added following 23 policies for AWS Auto remediation
